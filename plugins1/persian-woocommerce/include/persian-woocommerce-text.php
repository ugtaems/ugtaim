<?php
		...
		jQuery(document).ready(function($) {
			$("#pw_text").submit(function(){
				...
				jQuery.post(ajaxurl, $("#pw_text").serialize() , function(response) {
					...
					setTimeout(function () { $("#setting-error-pw_msg_"+ obj.rand).slideUp('slow' , function(){ $("#setting-error-pw_msg_"+ obj.rand).remove(); }) }, 3000);
	...
	function woocommerce_text_meta_box(){
		...
		<form action="" method="post" id="pw_text">
			<input type="hidden" name="s" value="<?php echo isset($_GET['s']) && !empty($_GET['s']) ? sanitize_text_field($_GET['s']) : ''; ?>" />
